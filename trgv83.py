			...
			for prod in prods:
				...
				for i in range(len(prod) - 1):
					...
					if prod[i] not in terminals:
						...
						follow_dict[prod[i]] = \
							follow_dict[prod[i]].union(FirstOF(First,
								prod[i + 1:]))
...
for x in predicates:
	...
	x=x.split()
	if len(x)<4:
		print("Predicate '"+x[0]+"' is of wrong format it should be in the form Name[Num]")
...
while top != "$":
	...
	if top == formula[Count]:
		...
		try:
			...
			exit()
	top = stack[-1]
print("formula is valid, parse tree generated. you can find tree in parse_tree.png ")
